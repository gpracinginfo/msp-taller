...
export function JobCard({
  ...
  return (
    <article
      ...
      } ${isDragging ? 'opacity-60 ring-2 ring-[#256d5a]' : ''} ${isDavid ? 'ring-2 ring-fuchsia-500 shadow-fuchsia-200' : ''} ${isFane && !isDavid ? 'ring-2 ring-orange-400 shadow-orange-200' : ''}`}
    >
      ...
      <div className="p-2.5">
        ...
        <div className="mt-2 flex gap-1" onClick={(event) => event.stopPropagation()}>
          <button
            type="button"
            className="rounded-lg border border-[#d8e5df] bg-white p-1.5 text-xs font-bold hover:bg-[#f6faf8]"
